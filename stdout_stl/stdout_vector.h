#ifndef __STDOUT_VECTOR_H_
#define __STDOUT_VECTOR_H_

#include "stdout_config.h"
#include "stdout_uninitialized.h"
#include "stdout_iterator.h"
#include "stdout_alloc.h"

STDOUT_START

// vector 的iterator 是一个randomAccessIterator
...
class _Vector_base {
    ...
    protected:
        _Tp* _M_start;              // 指向目前已使用空间头部
        _Tp* _M_finish;             // 指向目前已使用空间尾部
        _Tp* _M_end_of_storage;     // 指向目前可使用空间尾部
...
class vector : protected _Vector_base<_Tp, _Alloc> {
    ...
    public:
        ...
        // 构造函数
        // 利用 uninitialized_fill_n 填充 n 个
        ...
        vector(_InputIterator __first, _InputIterator __last, 
            ...
            typedef typename __Is_Integer<_InputIterator>::__Integral _Integral;
        ...
        void swap(vector<_Tp, _Alloc>& __x) {
            stdout_stl::swap(_M_start, __x._M_start);
            stdout_stl::swap(_M_finish, __x._M_finish);
            stdout_stl::swap(_M_end_of_storage, __x._M_end_of_storage);
        }
...
vector<_Tp, _Alloc>& vector<_Tp, _Alloc>::operator=(const vector<_Tp, _Alloc>& __x) {
    if (&__x != this) {
        ...
        if (__xlen > capacity()) {
            ...
            uninitialized_copy(__x.begin() + __x.size(), __x.end(), _M_finish);
        }
...
void vector<_Tp, _Alloc>::_M_fill_insert(iterator __position, size_type __n, const _Tp& __x) {
    if (__n != 0) {
        if (size_type(_M_end_of_storage - _M_finish) >= 0) {
            _Tp __x_copy = __x;
...
void vector<_Tp, _Alloc>::_M_range_insert(iterator __position, _ForwardIterator __first, _ForwardIterator __last, forward_iterator_tag) {
    if ( __first != __last) {
        ...
        if (size_type(_M_end_of_storage - _M_finish) >= __n) {
            ...
            destroy(_M_start, _M_end_of_storage - _M_start);
            _M_deallocate(_M_start, _M_end_of_storage - _M_start);
            _M_start = __new_start;
            _M_finish = __new_finish;
            _M_end_of_storage = __new_start + __len;
